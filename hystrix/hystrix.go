...
func CircuitBreakerWrapper(ctx *gin.Context) {
	...
	hystrix.Do(name, func() error {
		...
		if err != nil {
			// 监控上报（未实现）
			_, err1 := io.WriteString(f, fmt.Sprintf("circuitBreaker and err is %s\n", err.Error())) //写入文件(字符串)
			if err1 != nil {
...
func init() {
	hystrix.ConfigureCommand(CircuitBreakerName, hystrix.CommandConfig{
		...
		MaxConcurrentRequests:  2,                    // command的最大并发量
		RequestVolumeThreshold: 20,                   // 统计窗口10s内的请求数量，达到这个请求数量后才去判断是否要开启熔断
